(function () {
  ...
  function bindConversationRow(row) {
    ...
    row.addEventListener("contextmenu", (event) => openConversationMenu(event, row));
  ...
  conversationList.querySelectorAll(".convo-row").forEach((row) => {
    bindConversationRow(row);
  });

  projectList.querySelectorAll(".project-row").forEach((row) => {
    row.addEventListener("click", () => {
      setActiveProjectRow(row.dataset.projectId);
      state.activeProjectId = row.dataset.projectId;
      startNewChat();
      titleEl.textContent = "New chat";
      projectPillEl.style.display = "inline";
      projectPillEl.textContent = row.querySelector(".row-label").textContent;
    });
  });
  ...
  document.getElementById("new-project-form").addEventListener("submit", async (e) => {
    ...
    if (res.ok) {
      ...
      `;
      row.addEventListener("click", () => {
        setActiveProjectRow(project.id);
        state.activeProjectId = project.id;
        startNewChat();
        projectPillEl.style.display = "inline";
        projectPillEl.textContent = project.name;
      });
